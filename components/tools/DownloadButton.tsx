  ...
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-emerald-200 bg-emerald-50 p-5 text-center">
      <div>
        ...
        <p className="mt-1 font-semibold text-emerald-900">Your file is ready</p>
        <p className="truncate text-sm text-emerald-700">{filename}</p>
        <p className="mt-1 text-sm text-emerald-700">
          {formatBytes(blob.size)}
          {originalSize ? ` · was ${formatBytes(originalSize)}` : ""}
          {savings !== null && savings > 0 ? ` · ${savings}% smaller` : ""}
          {savings !== null && savings < 0 ? ` · ${Math.abs(savings)}% larger` : ""}
        </p>
      </div>
